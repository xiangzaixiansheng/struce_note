...
- [Go 语言实战:  编写可维护 Go 语言代码建议](#go-语言实战--编写可维护-go-语言代码建议)
  - [目录](#目录)
  - [介绍](#介绍)
  - [编者的话](#编者的话)
  - [正文](#正文)
    - [1. 指导原则](#1-指导原则)
      ...
      - [1.3. 生产力](#13-生产力)
    - [2. 标识符](#2-标识符)
      - [2.1. 选择标识符是为了清晰，而不是简洁](#21-选择标识符是为了清晰而不是简洁)
      - [2.2. 标识符长度](#22-标识符长度)
      - [2.2.1. 上下文是关键](#221-上下文是关键)
      - [2.3. 不要用变量类型命名你的变量](#23-不要用变量类型命名你的变量)
      - [2.4. 使用一致的命名方式](#24-使用一致的命名方式)
      - [2.5. 使用一致的声明样式](#25-使用一致的声明样式)
      - [2.6. 成为团队合作者](#26-成为团队合作者)
    - [3. 注释](#3-注释)
      - [3.1. 关于变量和常量的注释应描述其内容而非其目的](#31-关于变量和常量的注释应描述其内容而非其目的)
      - [3.2. 公共符号始终要注释](#32-公共符号始终要注释)
      - [3.2.1. 不要注释不好的代码，将它重写](#321-不要注释不好的代码将它重写)
      - [3.2.2. 与其注释一段代码，不如重构它](#322-与其注释一段代码不如重构它)
    - [4. 包的设计](#4-包的设计)
      - [4.1. 一个好的包从它的名字开始](#41-一个好的包从它的名字开始)
      - [4.1.1. 好的包名应该是唯一的。](#411-好的包名应该是唯一的)
    ...
    - [5. 项目结构](#5-项目结构)
      ...
      - [5.1.1. 通过 `import` 语句将代码排列到文件中](#511-通过-import-语句将代码排列到文件中)
      - [5.1.2. 优先内部测试再到外部测试](#512-优先内部测试再到外部测试)
      - [5.1.3. 使用 `internal` 包来减少公共API](#513-使用-internal-包来减少公共api)
      - [5.2. 确保 `main` 包内容尽可能的少](#52-确保-main-包内容尽可能的少)
    - [6. API 设计](#6-api-设计)
      - [6.1. 设计难以被误用的 API](#61-设计难以被误用的-api)
      - [6.1.1. 警惕采用几个相同类型参数的函数](#611-警惕采用几个相同类型参数的函数)
      - [6.2. 为其默认用例设计 API](#62-为其默认用例设计-api)
      - [6.2.1. 不鼓励使用 `nil` 作为参数](#621-不鼓励使用-nil-作为参数)
      - [6.2.2. 首选可变参数函数而非 `[]T` 参数](#622-首选可变参数函数而非-t-参数)
      - [6.3. 让函数定义它们所需的行为](#63-让函数定义它们所需的行为)
    - [7. 错误处理](#7-错误处理)
      - [7.1. 通过消除错误来消除错误处理](#71-通过消除错误来消除错误处理)
      - [7.1.1. 计算行数](#711-计算行数)
      - [7.1.2. WriteResponse](#712-writeresponse)
      - [7.2. 错误只处理一次](#72-错误只处理一次)
      - [7.2.1. 为错误添加相关内容](#721-为错误添加相关内容)
      - [7.2.2. 使用 `github.com/pkg/errors` 包装 `errors`](#722-使用-githubcompkgerrors-包装-errors)
    - [8. 并发](#8-并发)
